...
class TestBuildWithDependancies(object):
...
class TestRuntimeError:
    ...
    def test_exception_on_invalid_task_name(self):
        ...
        with pytest.raises(Exception) as exc:
            _pynt.build(build_with_params,["doesnt_exist"])
...
class TestPartialTaskNames:
    ...
    def test_exception_on_conflicting_partial_names(self):
        ...
        assert 'Conflicting matches clean, copy_file for task c' in exc.value.args[0]
...
class TesttaskArguments:
    ...
    def test_passing_varargs_and_keyword_args(self):
        ...
        # Use of OrderedDict.
        assert ['echo[1,2,3,some_str,111=333,bar=123.3,foo=xyz]'] ==  self._mod.tasks_run
    ...
    def test_validate_keyword_arguments_always_after_args(self):
        ...
        assert "Non keyword arg foo cannot follows" \
            " a keyword arg bar=123.3" in exc.value.args[0]

        with pytest.raises(Exception) as exc:
            _pynt.build(self._mod, ['copy[from_=/foo,/foo1]'])

        assert "Non keyword arg /foo1 cannot follows" \
            " a keyword arg from_=/foo" in exc.value.args[0]
    ...
    def test_invalid_number_of_args(self):
        ...
        assert 'takes 2 positional arguments but 3 were given' in exc.value.args[0]


    def test_invalid_names_for_kwargs(self):
        with pytest.raises(TypeError) as exc:
            _pynt.build(self._mod, ['copy[1=2,to=bar]'])
        assert "got an unexpected keyword argument '1'" in exc.value.args[0]

        with pytest.raises(TypeError) as exc:
            _pynt.build(self._mod, ['copy[bar123=2]'])
        assert "got an unexpected keyword argument 'bar123'" in exc.value.args[0]
